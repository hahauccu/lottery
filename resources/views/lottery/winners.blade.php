...
<body class="min-h-screen bg-gray-950 text-white">
    <div class="mx-auto max-w-5xl space-y-6 px-6 py-10">
        ...
        @if($needsVerification ?? false)
            {{-- Email 驗證表單 --}}
            <section class="rounded-2xl bg-gray-900/80 p-8 shadow text-center">
                <h2 class="text-xl font-semibold mb-2">請驗證您的身份</h2>
                <p class="text-gray-400 mb-6">輸入您的公司 Email 以查看中獎名單</p>

                <form method="POST" action="{{ route('lottery.winners.verify-email', ['brandCode' => $event->brand_code]) }}" class="max-w-md mx-auto space-y-4">
                    @csrf
                    <div>
                        <input
                            type="email"
                            ...
                            value="{{ old('email') }}"
                            class="w-full px-4 py-3 bg-gray-900/80 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition"
                        >
                        @error('email')
                            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    <button type="submit" class="w-full px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-semibold rounded-lg shadow-lg transition">
                        驗證並查看
                    </button>
                </form>
            </section>
                            ...
                            <div class="flex items-center justify-between px-4 py-3">
                                ...
                                <span class="text-sm text-gray-400">
                                    ...
                                    @if($winner->employee?->phone)
                                        · {{ \App\Support\DataMasker::maskPhone($winner->employee?->phone) }}
                                    @endif
                                </span>
                            </div>
                        @empty
