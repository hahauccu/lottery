...
<div class="wrapper" style="height: 90vh; ">
		...
		<div class="row">
			...
			<div class="span30" style="width: 70%">
				...
				<div  style="display:block height:7vh;">
					...
					<div class="sub_menu_button" div-name='lotterysteps'>
						<span>獎項管理</span>
					</div>
				...
				<div class="content lotterysteps" style="background-color: white; display:none;">
					<input type="button" value="新增" class='new_table_data' style="display: none">
				<table class='hidden_header' style='display:none;'>
				
					<td>
						<input type='text' column-name='step_name'> 
						<input type='hidden' column-name='id'>
					</td>
					<td><input type='text' column-name='prize_number'></td>
					<td>
						<select column-name='participate_list'>
							<option value="0">請選擇</option>
							@foreach($participantList as $participantListValue)
							
							<option value="{{$participantListValue['id']}}">{{$participantListValue['name']}}</option>
							@endforeach
						</select>
					</td>
					<td>
					...
					<td>
						<input type="checkbox" column-name="is_repeat_draw" value='1'>
					</td>
					<td>
						<div>
	...
	<script>
		...
		$(".sub_menu_button").click(function(){
			...
			$.ajax({
				url: '/adminer/sub_menu/{"where":{"lottery_code":"'+$("form").attr('action').split("/").pop()+'"},"target":"'+_this.attr('div-name')+'"}', 
				method:'get',
		...
		$("body").on('click','.save',function(){
			...
			data['target'] = 'lotterysteps';
			data['lottery_code'] = $("form").attr('action').split("/").pop();
		...
		$("body").on('click','.delete',function(){
			...
			if (yes) 
			{
				$.ajax({
					...
					url:'/adminer/sub_menu/'+'{"where":{"id":"'+_this.parent().find("input[column-name='id']").val()+'"},"target":"lotterysteps"}',
					data:
					{
